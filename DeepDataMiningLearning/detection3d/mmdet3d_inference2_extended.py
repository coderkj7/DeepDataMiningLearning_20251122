...
def visualize_with_open3d(lidar_file, predictions_dict, gt_bboxes, out_dir, basename,
    ...
    if img_file and calib_file:
        try:
            ...
            draw_projected_boxes_on_image(img_file, calib_file, pred_bboxes_tensor, gt_bboxes, str(img_2d_vis_path),
...
def bev_iou_aligned(box1, box2):
    ...
    box: [x, y, z, l, w, h, yaw]
    We ignore yaw here for simplicity.
    """
    x1, y1, _, l1, w1, _, _ = box1
    x2, y2, _, l2, w2, _, _ = box2
...
def accumulate_frame_metrics(pred_bboxes, pred_scores, gt_bboxes,
                             iou_thr=0.5, min_score=0.0,
                             all_scores=None, all_tp=None, all_fp=None,
                             gt_count=0):
    """
    Greedy matching: sort preds by score, match each to best GT (if IoU>=thr).
    Updates global arrays for AP-like analysis.

    Returns updated all_scores, all_tp, all_fp, gt_count.
    """
    if all_scores is None:
        all_scores, all_tp, all_fp = [], [], []
    gt_used = np.zeros(len(gt_bboxes), dtype=bool)

    pred_bboxes = np.array(pred_bboxes)
    pred_scores = np.array(pred_scores)

    # Filter low-scoring preds
    if len(pred_bboxes) == 0 or len(pred_scores) == 0:
        gt_count += len(gt_bboxes)
        return all_scores, all_tp, all_fp, gt_count
...
def summarize_detection_metrics(all_scores, all_tp, all_fp, gt_count):
    """
    Compute precision/recall curve and a crude AP estimate.
    """
    if gt_count == 0 or len(all_scores) == 0:
        print("[Metrics] No GT boxes or predictions to evaluate.")
        return
    ...
    for t in np.linspace(0, 1, 11):
        ...
        ap += (p.max() if p.size > 0 else 0.0) / 11.0

    print("\n[Detection Metrics]")
    print(f"  GT boxes: {gt_count}")
    print(f"  Predictions: {len(all_scores)}")
    print(f"  Final precision: {precision[-1]:.3f}")
    print(f"  Final recall:    {recall[-1]:.3f}")
    print(f"  Approx AP@0.5 (BEV-aligned): {ap:.3f}")
...
def main(args):
    ...
    for single_input in inputs_list:
        ...
        gt_bboxes_3d = []
        if 'gt_label' in single_input and single_input['gt_label'] is not None:
            try:
        ...
        # Track max GPU memory if PyTorch + CUDA available
        if torch is not None and torch.cuda.is_available():
        ...
        if len(pred_bboxes_3d) > 0 and len(pred_scores_3d) > 0:
            all_scores, all_tp, all_fp, gt_count = accumulate_frame_metrics(
                pred_bboxes_3d,
                pred_scores_3d,
                gt_bboxes_3d,
                iou_thr=0.5,
                min_score=args.score_thr,
            ...
            gt_count += len(gt_bboxes_3d)

        # --- Generate 2D Visualization (if img and calib are available) ---
        if (
            'img' in single_input and 'calib' in single_input
            and single_input['img'] is not None and single_input['calib'] is not None
        ):
    ...
    if len(per_frame_times) > 0:
        ...
        std_time = float(np.std(per_frame_times))
        fps = 1.0 / avg_time if avg_time > 0 else 0.0
        print("\n[Timing]")
        print(f"  Avg latency per frame: {avg_time*1000:.2f} ms")
        print(f"  Std latency per frame: {std_time*1000:.2f} ms")
        print(f"  Approx FPS: {fps:.2f}")
    if torch is not None and torch.cuda.is_available():
        print(f"[Memory] Max GPU memory allocated: {max_gpu_mem:.1f} MB")
